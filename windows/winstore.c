...
/* FEATURE: global storage type */
static enum storage_t storagetype = STORAGE_FILE;
...
# define invalid_filename(FindFileData) \
	((FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) || FindFileData.cFileName[0] == '.')

bool file_enum_settings_next(settings_e *e, strbuf *sb)
{
    ...
    if (!e) return false;    /* JK: new in 0.1.3 */
    ...
    if (!e->fromFile) {
        ...
        /*if (RegEnumKey(e->key, e->i++, otherbuf, 3 * buflen) == ERROR_SUCCESS) {
            ...
            while (invalid_filename(FindFileData)) { // HACK: PUTTY TRAY / PUTTY FILE: Fixed directory check
            ...
            /* JK: a file found */
            if (hFile != INVALID_HANDLE_VALUE && !invalid_filename(FindFileData)) { // HACK: PUTTY TRAY / PUTTY FILE: Fixed directory check
                e->hFile = hFile;
				//sgrowarray(buf, buflen, strlen(FindFileData.cFileName));
	    ...
	    if (invalid_filename(FindFileData)) {
                return enum_settings_next(e, sb);
	    }
		unescape_registry_key(FindFileData.cFileName, sb);
            //unmungestr(FindFileData.cFileName, buffer, buflen);
            //sfree(otherbuf);
...
#define file_verify_host_key reg_verify_host_key
int file_verify_host_key_(const char *hostname, int port,
		    const char *keytype, const char *key)
{
	...
	// fixme: wrong!! hostkey_regname expect strbuf*
    hostkey_regname(regname, hostname, port, keytype);
...
#define file_store_host_key reg_store_host_key
void file_store_host_key_(const char *hostname, int port,
		    const char *keytype, const char *key)
{
    char *regname = NULL;
    WIN32_FIND_DATA FindFile;
